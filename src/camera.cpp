...
void camera_free_poll_keys(Camera& cam, const bool *kb_state, float dt)
{
    ...
    if (kb_state[SDL_SCANCODE_PAGEUP]) {
        cam.pos = vec3_add(cam.pos, vec3_scale(cam.up, dt * ms));
        cam.at = vec3_add(cam.at, vec3_scale(cam.up, dt * ms));
        camera_update(cam);
    }
    if (kb_state[SDL_SCANCODE_PAGEDOWN]) {
        cam.pos = vec3_sub(cam.pos, vec3_scale(cam.up, dt * ms));
        cam.at = vec3_sub(cam.at, vec3_scale(cam.up, dt * ms));
        camera_update(cam);
    }
}
